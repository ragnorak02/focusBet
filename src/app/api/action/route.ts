...
import { newId, readDb, resetDb, updateDb } from '@/lib/db';
import { buildAppState } from '@/lib/appState';
import { computeBankroll, gradeAll, gradeBet } from '@/lib/engine';
import { round2 } from '@/lib/odds';
import { fetchEvent, matchBout, type EspnEvent } from '@/lib/espn';
import type { Corner, DB, Fight, MmaEvent, Outcome, Segment } from '@/lib/types';
...
/** Available balance right now, so we can reject bets the bankroll can't cover. */
async function currentBalance(db: DB): Promise<number> {
  return computeBankroll(db, gradeAll(db)).balance;
}

function applyEspnToEvent(ev: MmaEvent, remote: EspnEvent) {
  ...
  for (const fight of ev.fights) {
    ...
    // Corner order can differ between sources; flip the outcome to match ours.
    const nextStatus = bout.status;
    if (fight.status !== nextStatus) fight.status = nextStatus;
    ...
    // Never silently overwrite a result the user graded by hand.
    ...
    const winner =
      outcome === 'a' ? fight.a.name : outcome === 'b' ? fight.b.name : null;
    changes.push(
      winner
        ? `${winner} def. ${outcome === 'a' ? fight.b.name : fight.a.name}` +
          `${bout.method ? ` (${bout.method}` : ''}${bout.round ? `, R${bout.round} ${bout.time ?? ''}` : ''}${bout.method ? ')' : ''}`
...
export async function POST(req: Request) {
  ...
  try {
    /* ---- actions that don't mutate through updateDb ---- */
    if (type === 'resetAll') {
      const db = await resetDb();
      return NextResponse.json({ state: buildAppState(db) });
    }

    let message: string | undefined;
    let extra: Record<string, unknown> = {};

    const { db } = await updateDb(async (db) => {
      switch (type) {
        case 'deposit': {
        ...
        case 'withdraw': {
          const amount = money(p.amount, 'Amount');
          const bal = await currentBalance(db);
          if (amount > bal) {
            bad(`Can't cash out $${amount.toFixed(2)} — balance is $${bal.toFixed(2)}`);
          }
          db.cash.push({
            id: newId('cash'),
            at: new Date().toISOString(),
        ...
        case 'placeBet': {
          ...
          // Two picks in the same fight would be a guaranteed-loss parlay.
          ...
          const bal = await currentBalance(db);
          if (stake > bal) {
            bad(`Not enough funds — balance is $${bal.toFixed(2)}`);
          }
        ...
        case 'cashOutBet': {
          ...
          const graded = gradeBet(bet, db.events);
          if (graded.status !== 'open') bad('Only open bets can be cashed out');
          const amount = money(p.amount, 'Amount');
        ...
        case 'deleteFight': {
          ...
          // Bets on it become void automatically (gradeLeg handles a missing fight).
        ...
        case 'importEspnEvent': {
          ...
          if (existing) {
            const changes = applyEspnToEvent(existing, remote);
            extra.eventId = existing.id;
        ...
        case 'refreshResults': {
          ...
          // Prefer the linked ESPN event; otherwise find it by name/date.
          let remote: EspnEvent | null = null;
          if (ev.espnId) remote = await fetchEvent(ev.espnId);
          if (!remote) {
            const { fetchScoreboard } = await import('@/lib/espn');
          ...
          if (!remote) {
            bad(
              'Could not find this event on the ESPN feed. Link it by ESPN id, or grade the fights manually.',
            );
          }

          const changes = applyEspnToEvent(ev, remote);
          extra.changes = changes;
          message = changes.length
    ...
    return NextResponse.json({ state: buildAppState(db), message, ...extra });
  } catch (err) {
    const msg = err instanceof Error ? err.message : 'Something went wrong';
    const status = err instanceof ActionError ? 400 : 500;
    if (status === 500) console.error('[action]', type, err);
    // Return current state so the UI stays consistent after a rejected action.
    const db = await readDb();
    return NextResponse.json({ error: msg, state: buildAppState(db) }, { status });
  }
}
